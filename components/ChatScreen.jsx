...
export default function ChatScreen({
  ...
  const [handleBackPress, setHandleBackPress] = useState(false);

  BackHandler.addEventListener('hardwareBackPress',()=>{
    setHandleBackPress(!handleBackPress);
  })
    ...
    const fetchUserData = async () => {
      try {
        ...
        if (!querySnapshot.empty) {
          const userDoc = querySnapshot.docs[0]; // Get the first document if matched
  ...
  useEffect(() => {
    const fetchOtherUsers = async () => {
      ...
      try {
        const getUserSnapshot = await firestore()
          .collection('users')
          .where('fivaId', '!=', fivaId)
          .get();
            ...
            <View style={styles.userInfoCover}>
              <View style={styles.userProfile}>
                <TouchableOpacity
                  ...
                  <Image style={styles.userImage} source={{uri: pfp}} />
                </TouchableOpacity>
              ...
              <Image
                ...
                source={{
                  uri: 'https://static.vecteezy.com/system/resources/previews/014/455/886/non_2x/share-icon-on-transparent-background-free-png.png',
                }}
              />
            </View>
