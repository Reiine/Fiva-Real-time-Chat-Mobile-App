...
export default function Register({navigation}) {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      if (userDoc.exists) {
        Alert.alert('User already exists. Please try a different Fiva ID.');
        setLoading(false);
        return;
      }
      ...
      const salt = generateUserId(); 

      const userDocId = generateUserId();

      await firestore()
        .collection('users')
        .doc(userDocId)
        .set({
          fivaId: trimmedFivaId,
          username: username, 
  ...
  const pickImageAndUpload = async () => {
    try {
      ...
      Alert.alert(
        'Error',
        'An error occurred while picking or uploading the image.',
      );
