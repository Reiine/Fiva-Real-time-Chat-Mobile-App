...
export default function Login({navigation, setChangeState, changeState}) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const userRef = firestore().collection('users').where('fivaId', '==', trimmedFivaId);
      ...
      const userData = userDoc.docs[0].data(); // Get the user data
  
      // Decode the stored password from Base64
      const decodedPassword = Base64.decode(userData.password); // Decode the stored password
      ...
      navigation.navigate('Home');  // Adjust 'Home' to the appropriate screen name
